...
class ObjectVisualizer:
    def __init__(
        ...
        if self.display_plots["3d_separate"]:
            ...
            self.axes["3d_separate"] = self.figs["3d_separate"].add_subplot(
                111, projection="3d"
            )
            ...
            def plot_object(plot_type, ax):
                ...
                if plot_type in ["3d", "3d_separate"]:
                    ...
                    self.object_plots[plot_type][obj_id] = ax.scatter(
                        scatter_x, scatter_y, scatter_z, color=color, alpha=0.7, s=200
                    )
                    ...
                    scatter_x = (
                        x
                        if plot_type in ["xy", "xy_separate", "xz", "xz_separate"]
                        else y
                    )
                    scatter_y = (
                        z
                        if plot_type in ["xz", "xz_separate", "yx", "yx_separate"]
                        else y
                    )

                    self.object_plots[plot_type][obj_id] = ax.scatter(
                        scatter_x, scatter_y, color=color, alpha=0.7, s=200
                    )
                    ...
                    image_box = OffsetImage(icon, zoom=0.03)
                    self.icons[plot_type][obj_id] = ax.add_artist(
                        AnnotationBbox(
                            image_box,
                ...
                traj_x = (
                    x
                    if plot_type
                    in ["3d", "3d_separate", "xy", "xy_separate", "xz", "xz_separate"]
                    else y
                )
                traj_y = (
                    z
                    if plot_type
                    in ["3d", "3d_separate", "xz", "xz_separate", "yx", "yx_separate"]
                    else y
                )
