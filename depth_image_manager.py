...
class DepthManager():
    ...
    def update_disparity_map(self, left_image, right_image):
        ...
        right_image = cv2.cvtColor(cv2.LUT(right_image, lookUpTable),cv2.COLOR_RGB2GRAY)
        ...
        self.disparity_normalized = cv2.normalize(self.disparity, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    ...
    def position_img_2d_to_world_3d(self, position):
        ...
        average_position = np.mean(
            self.points[position[1] -2:position[1] + 2,
            position[0] - 2 : position[0] + 2],axis = (0,1))
